...
//     voiceActors: number[];
// }) {
//     // Функція для перевірки та додавання записів
//     const insertIfNotExists = async (table: SQLiteTable, conditions:  SQL<unknown>[], data: Record<string, unknown>) => {
//         const existingRecords = await db.select().from(table).where(and(...conditions)).execute();
//
...
//                 animeGenreTable,
//                 [eq(animeGenreTable.animeId, animeId), eq(animeGenreTable.genreId, genreId)],
//                 { animeId, genreId }
//             );
//         }
//
//         // Перекладачі
//         for (const memberId of translators) {
//             await insertIfNotExists(
//                 animeTranslateTable,
//                 [eq(animeTranslateTable.animeId, animeId), eq(animeTranslateTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Редактори
//         for (const memberId of editors) {
//             await insertIfNotExists(
//                 animeEditingTable,
//                 [eq(animeEditingTable.animeId, animeId), eq(animeEditingTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Звукорежисери
//         for (const memberId of soundDesigners) {
//             await insertIfNotExists(
//                 animeSoundTable,
//                 [eq(animeSoundTable.animeId, animeId), eq(animeSoundTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Візуальні дизайнери
//         for (const memberId of visualArtists) {
//             await insertIfNotExists(
//                 animeVideoEditingTable,
//                 [eq(animeVideoEditingTable.animeId, animeId), eq(animeVideoEditingTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Режисери дубляжу
//         for (const memberId of dubDirectors) {
//             await insertIfNotExists(
//                 animeDubDirectorTable,
//                 [eq(animeDubDirectorTable.animeId, animeId), eq(animeDubDirectorTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Вокалісти
//         for (const memberId of vocals) {
//             await insertIfNotExists(
//                 animeVocalsTable,
//                 [eq(animeVocalsTable.animeId, animeId), eq(animeVocalsTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
//         // Актори озвучки
//         for (const memberId of voiceActors) {
//             await insertIfNotExists(
//                 animeVoiceActorsTable,
//                 [eq(animeVoiceActorsTable.animeId, animeId), eq(animeVoiceActorsTable.memberId, memberId)],
//                 { animeId, memberId }
//             );
//         }
//
